...
using CadastroClientes.Models.AccountViewModels;
...
namespace CadastroClientes.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpGet]
        [AllowAnonymous]
        public IActionResult CreatePassword(string code = null)
        {
            if (code == null)
            {
                throw new ApplicationException("A code must be supplied for password reset.");
        ...
        public async Task<IActionResult> CreatePassword(ResetPasswordViewModel model)
        {
            ...
            if (user == null)
            {
                ModelState.AddModelError(null, "Usuário não encontrado");
            }
            else
            {
                ...
                if (result.Succeeded)
                {
                    var resultLogin = await _signInManager.PasswordSignInAsync(user.UserName, model.Password, false, lockoutOnFailure: false);
                    ...
                    return RedirectToAction(nameof(CreatePasswordConfirmation));
                }
                AddErrors(result);
            }
